...
#'
#' @param adjacency_matrix A boolean matrix, with element (*i*,*j*) set to TRUE if location *j* is adjacent to location *i*.
...
#' It's a good idea to keep it to the number of stops on the line you're dealing with.
#' @export
  ...
  for(i in stations) {
    ...
    if (type == "connected_B") {
      while(TRUE){
        ...
        if(sum(!is.na(dist_mod)) == 0) break # if no adjacency_matrix station exist, stop.
